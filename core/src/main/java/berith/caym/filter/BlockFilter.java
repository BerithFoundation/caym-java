...
public class BlockFilter extends Filter<String> {
    ...
    public BlockFilter(Caym caym, Callback<String> callback) {
        super(caym, callback);
    }

    @Override
    BerithFilter sendRequest() throws IOException {
        return caym.berith().berithNewBlockFilter().send();
    }

    @Override
    void process(List<LogResult> logResults) {
        for (BerithLog.LogResult logResult : logResults) {
            if (logResult instanceof BerithLog.Hash) {
                String blockHash = ((BerithLog.Hash) logResult).get();
